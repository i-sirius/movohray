...
var Svitlohray = (function () {
  ...
  function newGame() { clearInterval(interval); board = E.generate.apply(null, E.sizes[size]); moves = 0; elapsed = 0; last = null; started = false; finished = false; assisted = false; recommended = null; render(); interval = setInterval(tick, 200); playGameSound("roundStart"); }
  ...
  function palette(pair) { var message = document.getElementById("svitlohrayPaletteMessage"); if (!validColors(pair)) { message.textContent = text("contrast"); } else { colors = pair.slice(); save("colors", colors); message.textContent = ""; paint(); } document.getElementById("svitlohrayOn").value = colors[0]; document.getElementById("svitlohrayOff").value = colors[1]; }
  ...
  function render() {
    ...
    ["on","off"].forEach(function(key,i){var l=el("label","",text(key),inputs), input=el("input","",undefined,l); input.type="color"; input.id=i ? "svitlohrayOff":"svitlohrayOn"; input.value=colors[i]; input.onchange=function(){var pair=colors.slice();pair[i]=input.value;palette(pair);};});
    var tools=el("div","svitlohray-actions",undefined,details); button(tools,"reset",function(){palette(defaults);},"svitlohrayReset"); button(tools,"random",function(){var choices=[["#613c83","#ffe5a3"],["#174f88","#bdf4ec"],["#8a304e","#ffdfab"],["#315b39","#f9dfed"]].filter(function(p){return p.join()!==colors.join();}); palette(choices[Math.floor(Math.random()*choices.length)]);},"svitlohrayRandom");
  ...
  return { open:open, localize:localize, locales:locales, validColors:validColors,
    ...
    snapshot:function(){return {board:board ? E.deserialize(E.serialize(board)):null,moves:moves,elapsed:elapsed,finished:finished,assisted:assisted};}
  };
}());
