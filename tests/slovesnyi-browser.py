...
REVISION = '0.6.7-20260908-c4'
CACHE = 'movohray-cache-v0.6.7-b20260908-c4'
BASELINE = os.environ.get('MOVOHRAY_BASELINE', 'v0.6.6a')
OLD_CACHE = 'movohray-cache-v0.6.7-b20260907-c2' if BASELINE == '03d572e' else 'movohray-cache-v0.6.6a-b20260825'
...
def static_audit():
    ...
    assert json.loads((ROOT / 'version.json').read_text()) == {'version': '0.6.7', 'build': '2026-09-08', 'candidate': 'c4', 'required': True}
...
def smoke(browser, base_url):
    ...
    assert 'Світлограй' not in page.locator('#appUpcomingModesChips').inner_text()
    assert page.locator('#svitlohrayOpen').count() == 1
...
def pwa(browser, base_url):
    ...
    wait_async(page, '''async () => {
      ...
      return r.active && r.active.scriptURL.includes('0.6.7-20260908-c4') && r.active.state === 'activated' && !r.installing;
    }''')
    assert page.evaluate('getLocalReleaseInfo().revision') == REVISION
    assert page.evaluate('normalizeReleaseInfo({version:"0.6.7",build:"2026-09-08",candidate:"c4"}).revision') == REVISION
    ...
    context.close()
    print('PWA PASS:', BASELINE, 'install -> c4 waiting -> activate; old cache removed; all revisioned assets; offline reload and play')
